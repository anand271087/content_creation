...
(MIT, see LICENSE). Upstream affiliate link removed; sign up at plain <https://arcads.ai>.
...
- **No `MASTER_CONTEXT.md` here** — where this skill says MASTER_CONTEXT.md,
  use the repo's `CLAUDE.md` + auto-memory instead (brand voice, defaults,
  credit costs). Never create a MASTER_CONTEXT.md.
- **Every downloaded video** must be re-encoded to H264 Baseline before any
  compositing (`ffmpeg -i in.mp4 -c:v libx264 -profile:v baseline -level 3.1
  -pix_fmt yuv420p -movflags +faststart -y out.mp4`) — same rule as Kling.
- **Arcads actor videos are talking-head MP4s** → they drop straight into the
  existing format library: transcribe with `speech/transcribe_elevenlabs.py`,
  then composite with any `formats/*.py` (tier boards, process_walkthrough,
  authority_stack…). Verify the actor's crop/framing per video first
  (letterbox check with cropdetect, like avatar looks — crops differ per actor).
- **Palette-from-background rule applies**: derive pill/caption/accent colors
  from the actor's set, dark pills behind text on warm/bright sets.
- **When to reach for Arcads vs HeyGen**: HeyGen = Anand's own avatar (the
  channel face). Arcads = other actors (UGC-style ads, skits, client work,
  A/B ad batches) and its extra models (Seedance/Sora/Veo/Kling/Nano Banana).
- **Credits are real money** — always show the credit estimate and get a yes
  before firing generation calls (the pack's confirm-before-fire flow below).
- Use `python3.11` for any Python here (system `python3` is a broken 3.14).

## Read order

1. This repo's **`CLAUDE.md`** (brand voice, format library, house rules).
