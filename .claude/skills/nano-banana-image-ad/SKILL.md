...
> Route all generation through **Kie.ai nano-banana-2** instead, which this
> repo already pays for (Stage 2): `POST https://api.kie.ai/api/v1/jobs/createTask`
> with `{"model": "nano-banana-2", "input": {"prompt": ..., "image_input": [refs],
> "aspect_ratio": ..., "resolution": "1K", "output_format": "png"}}`, header
> `Authorization: Bearer $KIE_API_KEY`; poll `GET /api/v1/jobs/task/{taskId}`,
> result at `resultJson.resultUrls[0]` (see CLAUDE.md API reference). The
> bundled `scripts/generate_image.py` targets Arcads — don't run it; keep every
> other phase of this skill (template selection, safety suffixes, edge-safe
> layout, QA) exactly as written. Use `python3.11` for any Python.
