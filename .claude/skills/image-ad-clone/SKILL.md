...
> Validation round-trips run on **Kie.ai nano-banana-2** (routing block in
> nano-banana-image-ad/SKILL.md). At Phase 1, always pick Nano Banana as the
> validation backend — gpt-image-2 is unavailable here. Everything else
> (analysis, template extraction, library append format) stays as written.
