...
> and gpt-image-2 has no Kie.ai equivalent here.** Treat this skill as a
> PROMPT LIBRARY: for typography-heavy / UI-mimicry templates, first try the
> same prompt on **Kie.ai nano-banana-2** (see nano-banana-image-ad's routing
> block); if body text renders garbled, build the UI mock as HTML/PIL instead
> (this repo's capture + cards stack renders pixel-perfect fake UIs for free —
> often better than any image model for Slack/Notes/search mockups).
