...
> **THIS REPO (2026-07-16): do NOT call the Arcads API — no plan is active.**
> Route generation through **Kie.ai nano-banana-2** (already paid, Stage 2):
> `POST https://api.kie.ai/api/v1/jobs/createTask` with `{"model":
> "nano-banana-2", "input": {"prompt": ..., "image_input": [face/brand refs],
> "aspect_ratio": "16:9" (or "9:16" for reel covers), "resolution": "1K"}}`,
> `Authorization: Bearer $KIE_API_KEY`, poll `/api/v1/jobs/task/{taskId}` →
> `resultJson.resultUrls[0]`. The 5 CTR formulas, likeness-reference flow, and
> batch A/B firing below stay exactly as written. For Anand's face use frames
> from an existing avatar render as the reference images. PIL composition
> (core/cards.py-style) remains the fallback for text-heavy covers — AI image
> text still misspells; keep headlines as PIL overlays when exact text matters.
